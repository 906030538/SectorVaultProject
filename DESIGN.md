...
6. 索引分支允许所有人提交pr，门禁校验通过后自动合入。索引PR的分支策略：索引仓属主同仓开工作分支；其他用户fork索引仓（重复fork返回既有fork），既有fork可能落后于上游（缺新提交对象，直接以上游头建分支会422），提交前先把上游索引分支快进合并进fork（跨仓merge API），无法快进时退回fork自身分支头（提交内容基于上游最新归档构建，仍可干净合并）。
