...
9. 未登陆时，点击导航栏的投稿或登陆按钮，弹出页签式授权对话框（各平台页签+已登陆标记+面板内登出）。
...
2. 左侧列表目录（desktop吸附侧栏，窄屏横排回退），点击无刷新切换；目录列表来自deployment.json的`faqPages`配置。
...
### 编辑器
1. 新建投稿时，显示投稿仓库下拉框、slug输入框和工程、专栏互斥按钮；slug的placeholder显示为当前6位数字日期+标题；slug输入框有模式校验（与索引仓schema的pattern一致：字母数字下划线连字符假名汉字谚文）。编辑稿件时不可编辑。
...
8. 工程文件上传控件：每个文件提供方案选择（不处理/格式化/压缩/加密）。压缩/加密文件入库附加`.zip`后缀（显示名保持原名）。GitHub平台隐藏附件上传控件，显示文本提示。
9. 发布简介输入框（新建和编辑均显示；编辑时从release正文回填，修改后同步更新关联release）。
10. 附件上传控件（非GitHub平台）。
11. 投稿总是创建关联issue（不再提供评论区开关）。
12. 新建投稿流程：
    1. 创建slug同名issue（正文为稿件参数），记录issue编号（字符串）。
    2. 将封面、工程文件、README.md（含作者/邮箱/标签）、LICENSE（如有）写入slug目录，与仓库README链接和本地索引合并为一个提交。
    3. 创建slug同名release，正文包含发布简介和原仓库slug目录链接（非本站链接），记录release id（字符串）。
