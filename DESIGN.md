...
3. 如果已经登录的账户是当前用户，显示投稿、编辑和删除按钮。编辑弹窗：许可证选择（SPDX列表+自定义项，自定义显示全文输入框；未知/自定义许可证预选自定义项——select须挂载后赋值，WebKit未挂载时.value/.index均不可靠），选择不同许可证保存时将全文（自定义用输入文本，SPDX取标准全文）提交写入仓库根目录LICENSE；未变更直接关闭不提交。
4. 工程和专栏各一个标签按钮，切换显示。
