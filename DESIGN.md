...
2. `/view`、`/user`、`/edit` 动态路由不做构建期预渲染（getStaticPaths 返回空，不为具体用户/稿件生成静态页）：全部经 404 SPA 回退客户端渲染，索引更新无需重建站点即可访问新稿件。404 回退页内嵌即时脚本：路径命中 SPA 路由前缀（view/user/edit/discussions/login）时先隐藏 404 与返回按钮（避免模块脚本加载期间闪现），2.5 秒后仍无路由组件激活则恢复显示。
...
7. 全局导航左侧为站点logo与导航项（宽度不足时从右向左逐项折叠），右侧为线路选择（自定义下拉：收起仅显示平台名，展开标注global主要线路/国内镜像）、语言选择（翻译图标按钮+下拉，四语言）、主题选择（图标按钮随当前模式切换：半圆/太阳/月亮+下拉，选择即时生效）、合并菜单开关「⋯」（<lg显示）与投稿/用户区（桌面行内）。合并菜单面板中投稿与登录/用户信息置顶且同行并排（投稿弹性撑满剩余宽度），分隔线之下为已折叠的导航项；三个选择器任何宽度都常驻显示不折入下拉（≤430px时收紧按钮内边距与间距）。
   全局组件基类（btn/chip/card/input/prose-svp等）置于 @layer components：层序先于 utilities，hidden 等display工具类可覆盖（层外规则曾导致 btn/chip 元素无法隐藏）。
8. 线路选择后用户区按该平台取会话；无该平台登录信息时显示登录按钮（弹窗预选该平台）。
9. 未登陆时，点击导航栏的投稿或登陆按钮，弹出页签式授权对话框（各平台页签+已登陆标记+面板内登出）。导航栏登出按钮仅登出当前线路平台的账号（其他线路登录态保留）。已保存令牌失效时（匿名可用接口带令牌返回401，GitHub与gitee/atomgit/gitcode的V5系适配器均侦测）：自动清除该平台登录态并弹「登录过期」对话框，提供重新登录（打开授权对话框）或匿名访问（刷新页面）两个选项；每次页面加载每平台最多提示一次，登录流程中校验的候选令牌不触发。
...
3. 提供筛选器，可以根据关联曲目、合成引擎、使用声库、歌曲语言、许可证（SPDX标识/custom，按出现次数给候选）筛选。
4. 提供搜索器，输入关键字点搜索按钮后，遍历索引，列出关联稿件（关键字匹配标题、用户名、关联曲目与标签）。
5. 列表显示稿件封面、标题、投稿时间、用户名、关联曲目、合成引擎、使用声库、歌曲语言、有无参数。封面图与媒体元素一律 no-referrer（gitee raw 有 Referer 防盗链，跨站引用 403），且 referrerPolicy 必须先于 src 赋值设置。
6. 自动获取当前页的稿件关联issue的评论数和点赞数（👍表情数量），并缓存；互动统计按仓库分组各列出一次issues（此前逐稿件重复列出），列出载荷自带点赞数的平台（GitHub的reactions[+1]）把整仓issue互动（标题/编号/评论数/点赞数）写入localStorage缓存`svp-eng:{platform}:{owner}/{repo}`（TTL同current.json 10分钟，gitee/atomgit载荷无点赞数不缓存），命中时零请求。稿件详情页优先读该缓存构造关联issue（不再列出全部issues，未命中回退列出）；点赞/取消后同步增减缓存中的点赞数。
...
15. 内容仓数据失败（任意原因）时按索引mirrors.json记录依次尝试镜像仓（集合页：entries为空时读镜像svp-archive.json按原始owner/repo过滤；详情页：索引/线路兜底均失败时从镜像archive构造条目、或正文装载失败时整组改从镜像装载），镜像成功后线路切换为镜像平台（不重载——索引记录平台仍是源仓库，重载会被自动切线切回）；镜像仓的issue/release按标题/标签匹配slug（索引记录的id属源仓库不适用）。
...
2. 如果已经登录的账户是当前用户（无索引记录时也显示），显示新建集合按钮，在弹出界面引导用户创建新仓库。
   ...
   3. 提供模板库选择（deployment.json按平台配置，三平台均支持：GitHub走模板生成API，gitee/atomgit递归复制模板仓文本文件后单次提交）。
...
3. 如果已经登录的账户是当前用户，显示投稿、编辑和删除按钮。编辑弹窗：许可证选择（SPDX列表+自定义项，自定义显示全文输入框；未知/自定义许可证预选自定义项——select须挂载后赋值，WebKit未挂载时.value/.index均不可靠），选择不同许可证保存时将全文（自定义用输入文本，SPDX取标准全文）提交写入仓库根目录LICENSE；未变更直接关闭不提交。镜像仓库列表：与新建投稿页列表输入同款行式样式（行输入+×移除+行尾+添加、失焦校验红框、保存统一拦截非法地址，模式^(github|gitee|atomgit|gitcode)\.com/属主/仓库$），回填自svp-archive.json的mirrors（去除源仓库自身）；保存时按序[源仓库地址, ...镜像]写入svp-archive.json的mirrors字段，并向索引仓提单文件PR修改index/mirrors.json（源仓库地址为键、列表为值；源选择与集合同平台优先回退主源）。
4. 工程和专栏各一个标签按钮，切换显示。
...
13. 修改投稿流程：
    ...
    5. 附件同步到关联release。
    6. 索引属性有修改时更新本地归档并向索引仓提PR（含许可证变更）。索引归档序列化按key递归排序（stableStringify），编辑不改变字段顺序；发布时间输入为分钟精度，与原值同一分钟视为未改动以保留秒级精度。
...
2. 稿件索引记录：平台、属主、仓库、slug、标题、封面链接、投稿时间、发布时间、作者名、邮箱、许可证名称（SPDX标识，非SPDX统一为custom）、关联issue编号（字符串）、关联release id（字符串）、标签列表、关联曲目、合成引擎、使用声库、歌曲语言、有无参数。
...
7. 浏览时按线路加载对应平台索引，带本地缓存（current 10分钟、当月归档12小时、过往归档3天）；缓存键为文件路径（不带线路/平台信息——各线路索引互为镜像，切线时TTL内复用缓存）。
